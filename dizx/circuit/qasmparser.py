...
class QASMParser(object):
    ...
    def parse(self, s: str, strict:bool=True) -> Circuit:
        ...
        if match and match.group(1):
            ...
            raise ValueError("File does not start with supported OPENQASM descriptor.")
        ...
        if self.qasm_version == 2 and r[0].startswith('include "qelib1.inc";') or \
            ...
            raise ValueError("File is not importing standard library")
        
        if r[0].startswith('quditdim'):
            self.qudit_dim = int(r[0].split(' ')[1])
            r.pop(0)
        else:
            raise ValueError("File does not specify the qudit dimension via e.g. 'quditdim 5'.")
        ...
        circ = Circuit(self.qubit_count, self.qudit_dim)
    ...
    def extract_command_parts(self, c: str) -> Tuple[str,int,List[Fraction],List[str]]:
        ...
        if left_bracket != -1:
            if right_bracket == -1:
                raise ValueError(f"Mismatched bracket: {name}.")
            vals = name[left_bracket+1:right_bracket].split(',')
            phases = [self.parse_phase_arg(val) for val in vals]
            name = name[:left_bracket]
        i = name.find('^')
        if i != -1: # name = "gatename^repetitions"
            reps = int(name[i+1:])  # Currently the repetitions should be an integer. In the future this could also be a parameter.
            name = name[:i]
        else:
            reps = 1
        return name, reps, phases, args

    def parse_command(self, c: str, registers: Dict[str,Tuple[int,int]]) -> List[Gate]:
        gates: List[Gate] = []
        name, reps, phases, args = self.extract_command_parts(c)
        ...
        for j in range(dim):
            ...
            if name in self.custom_gates:
                ...
                g = qasm_gate_table[name](argset[0], phases[0], phases[1])  # type: ignore
                g.repetitions *= reps
                gates.append(g)  # type: ignore
            elif name in ('u3', 'u', 'U'):
                if len(phases) != 3: raise TypeError("Invalid specification {}".format(c))
                g = qasm_gate_table[name](argset[0], phases[0], phases[1], phases[2])  # type: ignore
                g.repetitions *= reps
                gates.append(g)  # type: ignore
                ...
                g.repetitions *= reps
                gates.append(g)
            elif name in ('cxdg', 'czdg'):
                if len(phases) != 0: raise TypeError("Invalid specification {}".format(c))
                g = qasm_gate_table[name](control=argset[0],target=argset[1])  # type: ignore
                g.adjoint = True
                g.repetitions = reps*-1
                gates.append(g)
